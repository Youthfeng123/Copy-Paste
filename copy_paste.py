...
def main(args):
    ...
    os.makedirs(os.path.join(args.output_dir, 'JPEGImages'), exist_ok=True)   #创建输出文件夹

    masks_path = os.listdir(segclass)      #列出文件夹下所有的文件名


    tbar = tqdm.tqdm(masks_path, ncols=100)  #进度条


    for mask_path in tbar:   #读取所有的mask
        # get source mask and img


        mask_src = np.asarray(Image.open(os.path.join(segclass, mask_path)), dtype=np.uint8)
        img_src = cv2.imread(os.path.join(JPEGs, mask_path.replace('.png', '.jpg')))   #读取图片
